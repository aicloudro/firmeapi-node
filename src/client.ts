...
const DEFAULT_BASE_URL = 'https://firmeapi.ro/api';
...
export class FirmeApi {
  ...
  /** Get company administrators by CUI */
  async getAdministratori(cui: string): Promise<any> {
    const cleanCui = this.cleanCui(cui);
    const response = await this.request<ApiResponse<any>>(`/v1/administratori/${cleanCui}`);
    return response.data;
  }

  /** Search work points with filters */
  async getPuncteLucru(filters: Record<string, any> = {}): Promise<any> {
    const params = new URLSearchParams();
    for (const [key, value] of Object.entries(filters)) {
      if (value !== undefined && value !== null) params.append(key, String(value));
    }
    const query = params.toString();
    const response = await this.request<ApiResponse<any>>(`/v1/puncte-lucru${query ? `?${query}` : ''}`);
    return response.data;
  }

  /** Get ARR transport licenses by CUI */
  async getArr(cui: string): Promise<any> {
    const cleanCui = this.cleanCui(cui);
    const response = await this.request<ApiResponse<any>>(`/v1/arr/${cleanCui}`);
    return response.data;
  }

  /** Get alternative transport authorizations by CUI */
  async getAlternativ(cui: string): Promise<any> {
    const cleanCui = this.cleanCui(cui);
    const response = await this.request<ApiResponse<any>>(`/v1/alternativ/${cleanCui}`);
    return response.data;
  }

  /** Get BPI insolvency publications by CUI */
  async getBpiCui(cui: string, options: Record<string, any> = {}): Promise<any> {
    const cleanCui = this.cleanCui(cui);
    const params = new URLSearchParams();
    ...
    const query = params.toString();
    const response = await this.request<ApiResponse<any>>(`/v1/bpi/cui/${cleanCui}${query ? `?${query}` : ''}`);
    return response.data;
  }

  /** Get BPI publications by case number */
  async getBpiDosar(numarDosar: string, options: Record<string, any> = {}): Promise<any> {
    const params = new URLSearchParams();
    for (const [key, value] of Object.entries(options)) {
      if (value !== undefined && value !== null) params.append(key, String(value));
    }
    const query = params.toString();
    const response = await this.request<ApiResponse<any>>(`/v1/bpi/dosar/${encodeURIComponent(numarDosar)}${query ? `?${query}` : ''}`);
    return response.data;
  }

  /** Search BPI publications */
  async getBpiSearch(searchQuery: string, options: Record<string, any> = {}): Promise<any> {
    const params = new URLSearchParams();
    for (const [key, value] of Object.entries(options)) {
      if (value !== undefined && value !== null) params.append(key, String(value));
    }
    const query = params.toString();
    const response = await this.request<ApiResponse<any>>(`/v1/bpi/cauta/${encodeURIComponent(searchQuery)}${query ? `?${query}` : ''}`);
    return response.data;
  }

  /** Get BPI publication by BPI number */
  async getBpiByNumber(numarBpi: string, options: Record<string, any> = {}): Promise<any> {
    const params = new URLSearchParams();
    for (const [key, value] of Object.entries(options)) {
      if (value !== undefined && value !== null) params.append(key, String(value));
    }
    const query = params.toString();
    const response = await this.request<ApiResponse<any>>(`/v1/bpi/${encodeURIComponent(numarBpi)}${query ? `?${query}` : ''}`);
    return response.data;
  }

  /** Get court cases with filters */
  async getDosare(filters: Record<string, any> = {}): Promise<any> {
    const params = new URLSearchParams();
    for (const [key, value] of Object.entries(filters)) {
      if (value !== undefined && value !== null) params.append(key, String(value));
    }
    const query = params.toString();
    const response = await this.request<ApiResponse<any>>(`/v1/dosar${query ? `?${query}` : ''}`);
    return response.data;
  }

  /**
   * Get basic company info using the free API (no API key required)
   *
   * @param cui - The company's CUI
